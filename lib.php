...
function local_movegroup_cron_process_file($filename, $filedir='') {
    ...
    while ($line = $cir->next()) {
        ...
        foreach ($filecolumns as $column) {
            ...
            if (!empty($user->{'group'.$i}) && !empty($user->ngroup)) {
                $upt->track("Group fields", "group1 & ngroup field not empty");
                $fromgroupname = $user->{'group'.$i};
                $togroupname = $user->ngroup;
                ...
                if ($isfromgroupnameexist == true && $istogroupnameexist == true) {
                    mtrace("</br> Enter group1 exist && ngroup exist");
                    ...
                    if ($isuserinfromgroupmember == true) {
                        try {
                            ...
                            if (groups_remove_member($isfromgroupnameexist, $user->id)) {
                                mtrace("</br> remove user from group1 successfully");
                                $upt->track('enrolments', "remove member from " . $fromgroupname . " successfully");
                            } else {
                                mtrace("</br> remove user from group1 unsuccessfully");
                                $upt->track('enrolments', "remove member from " . $fromgroupname . " unsuccessfully");
                            }
                        } catch (moodle_exception $e) {
                            mtrace("</br> remove user from group1 thrown exceptions");
                    ...
                    if ($isuserintogroupmember == true) {
                        ...
                        mtrace("</br> user is a member of ngroup, won't add to it!!!!");
                    } else {
                        try {
                            if (groups_add_member($istogroupnameexist, $user->id)) {
                                mtrace("</br> add user to new group1 successfully");
                                $upt->track('enrolments', get_string('addedtogroup', '', s($togroupname)));
                            } else {
                                mtrace("</br> add user to new group1 unsuccessfully");
                                $upt->track('enrolments', get_string('addedtogroupnot', '', s($togroupname)), 'error');
                            }
                        } catch (moodle_exception $e) {
                            mtrace("</br> add user to new group1 thrown exception");
                            $upt->track('enrolments', get_string('addedtogroupnot', '', s($togroupname)), 'error');
                            continue;
                        }
                    }
                    mtrace("</br> Out group1 exist && ngroup exist");
                }
                
                if ($isfromgroupnameexist == true && $istogroupnameexist == false) {
                    mtrace("</br> Enter group1 exist && ngroup not exist");
                    ...
                    mtrace("</br> Out group1 exist && ngroup not exist");
                }
                if ($isfromgroupnameexist == false && $istogroupnameexist == true) {
                    mtrace("</br> Enter group1 not exist && ngroup exist");
                    if ($isuserintogroupmember == true) {
                    ...
                    mtrace("</br> Out group1 not exist && ngroup exist");
                }
                if ($isfromgroupnameexist == false && $istogroupnameexist == false) {
                    mtrace("</br> Enter group1 not exist && ngroup not exist");
                    // Create new group and add user inside the group
                    // Build group cache
                    if (is_null($ccache[$shortname]->groups)) {
                        $ccache[$shortname]->groups = array();
                    ...
                    mtrace("</br> Out group1 not exist && ngroup not exist");
                }
            }
            
            if (empty($user->{'group'.$i})) {
                $upt->track("Group fields", "group1 field is empty", 'error');
            }
            if (empty($user->ngroup)) {
                $upt->track("Group fields", "ngroup field is empty", 'error');
            }
        }
    }
...
function local_movegroup_cron_values() {
    ...
    $STD_FIELDS = array('id', 'firstname', 'lastname', 'username', 'email',
            ...
            'url', 'description', 'descriptionformat', 'password', 'ngroup', // Kentest20180525
